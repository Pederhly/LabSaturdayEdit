...
void heli_q8_oppgave3_output0(void)    /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(heli_q8_oppgave3_M)) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      for (ci = 0; ci < 5; ci++) {
        heli_q8_oppgave3_B.Gain[i] += heli_q8_oppgave3_P.C_est[(ci << 2) + i] *
          rtb_StateSpace_0[ci];
      }
  ...
  for (i = 0; i < 4; i++) {
    phi = 0.0;
    for (ci = 0; ci < 5; ci++) {
      phi += heli_q8_oppgave3_P.C_est[(ci << 2) + i] *
        heli_q8_oppgave3_B.Integrator1[ci];
    }

    tmp_0[i] = heli_q8_oppgave3_B.Gain[i] - phi;
  }

  /* End of Sum: '<S3>/Sum2' */
  for (i = 0; i < 5; i++) {
    /* Gain: '<S3>/A_est' */
    rtb_StateSpace_0[i] = 0.0;
    for (ci = 0; ci < 5; ci++) {
      rtb_StateSpace_0[i] += heli_q8_oppgave3_P.A_est[5 * ci + i] *
        heli_q8_oppgave3_B.Integrator1[ci];
    }

    /* End of Gain: '<S3>/A_est' */

    /* Gain: '<S3>/L' */
    phi = heli_q8_oppgave3_P.L[i + 15] * tmp_0[3] + (heli_q8_oppgave3_P.L[i + 10]
      * tmp_0[2] + (heli_q8_oppgave3_P.L[i + 5] * tmp_0[1] +
                    heli_q8_oppgave3_P.L[i] * tmp_0[0]));
    ...
    heli_q8_oppgave3_B.Sum5[i] = (heli_q8_oppgave3_P.B_est[i + 5] * az +
      heli_q8_oppgave3_P.B_est[i] * rtb_Sum1) + (rtb_StateSpace_0[i] + phi);
  }
  ...
  if (rtmIsMajorTimeStep(heli_q8_oppgave3_M)) {
    ...
    for (i = 0; i < 5; i++) {
      rtb_TmpSignalConversionAtToFile[i + 27] = heli_q8_oppgave3_B.Integrator1[i];
    }
      ...
      if (rtmIsMajorTimeStep(heli_q8_oppgave3_M) ) {
        {
          if (!(++heli_q8_oppgave3_DW.ToFile_IWORK.Decimation % 1) &&
              (heli_q8_oppgave3_DW.ToFile_IWORK.Count * (32 + 1)) + 1 <
              100000000 ) {
            ...
            if (fp != (NULL)) {
              ...
              if (((++heli_q8_oppgave3_DW.ToFile_IWORK.Count) * (32 + 1))+1 >=
                  100000000) {
                (void)fprintf(stdout,
...
void heli_q8_oppgave3_terminate(void)
{
  ...
  /* Terminate for ToFile: '<S2>/To File' */
  {
    ...
    if (fp != (NULL)) {
      ...
      if (rt_WriteMat4FileHeader(fp, 32 + 1,
...
RT_MODEL_heli_q8_oppgave3_T *heli_q8_oppgave3(void)
{
  ...
  /* External mode info */
  heli_q8_oppgave3_M->Sizes.checksums[0] = (1582622131U);
  heli_q8_oppgave3_M->Sizes.checksums[1] = (4211664997U);
  heli_q8_oppgave3_M->Sizes.checksums[2] = (289972717U);
  heli_q8_oppgave3_M->Sizes.checksums[3] = (791755040U);
...

  {
    ...
    heli_q8_oppgave3_B.Gain[0] = 0.0;
    heli_q8_oppgave3_B.Gain[1] = 0.0;
    heli_q8_oppgave3_B.Gain[2] = 0.0;
    heli_q8_oppgave3_B.Gain[3] = 0.0;
  ...
  heli_q8_oppgave3_M->Sizes.numBlockPrms = (625);/* Sum of parameter "widths" */
